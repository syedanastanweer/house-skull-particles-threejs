...
function transformMesh(){
    // Increase the number of particles and reduce their size for a smoother appearance
    for (let i = 0; i < 200000; i++) { // Increase the number of particles
        sampler.sample(tempPosition)
        vertices.push(tempPosition.x, tempPosition.y, tempPosition.z)
    }
    pointsGeometry.setAttribute('position', new THREE.Float32BufferAttribute(vertices, 3))

    // Reduce the particle size for a smoother appearance
    const pointsMaterial = new THREE.PointsMaterial({
        color: 0xffff00, // Yellow color
        size: 0.02, // Smaller size for smoother particles
